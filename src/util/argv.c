 ...
 */
int cci_argv_append(int *argc, char ***argv, const char *arg)
{
    int rc;

    /* add the new element */
    if (CCI_SUCCESS != (rc = cci_argv_append_nosize(argv, arg))) {
	return rc;
    }
    *argc = cci_argv_count(*argv);

    return CCI_SUCCESS;
}

int cci_argv_append_nosize(char ***argv, const char *arg)
{
    int argc;
...
int cci_argv_append_unique_nosize(char ***argv, const char *arg, int overwrite)
{
    int i;

    /* if the provided array is NULL, then the arg cannot be present, so
       just go ahead and append */
    if (NULL == *argv) {
	return cci_argv_append_nosize(argv, arg);
    }
 ...
 */
static char **cci_argv_split_inter(const char *src_string, int delimiter,
				    int include_empty)
{
    char arg[ARGSIZE];
    char **argv = NULL;
	...
	if (src_string == p) {
	    if (include_empty) {
		arg[0] = '\0';
		if (CCI_SUCCESS != cci_argv_append(&argc, &argv, arg))
		    return NULL;
	    }
	}
	/* tail argument, add straight from the original string */
	    ...
	    if (CCI_SUCCESS != cci_argv_append(&argc, &argv, src_string))
		return NULL;
	    src_string = p;
	    continue;
	}
	/* long argument, malloc buffer, copy and add */
	    ...
	    if (CCI_SUCCESS != cci_argv_append(&argc, &argv, argtemp)) {
		free(argtemp);
		return NULL;
	    }
	    free(argtemp);
...
char **cci_argv_split(const char *src_string, int delimiter)
{
    return cci_argv_split_inter(src_string, delimiter, 0);
}

char **cci_argv_split_with_empty(const char *src_string, int delimiter)
{
    return cci_argv_split_inter(src_string, delimiter, 1);
}

/*
 * Return the length of a NULL-terminated argv array.
 */
int cci_argv_count(char **argv)
{
    char **p;
    int i;
 ...
 */
char *cci_argv_join(char **argv, int delimiter)
{
    char **p;
    char *pp;
    char *str;
 ...
 */
char *cci_argv_join_range(char **argv, size_t start, size_t end, int delimiter)
{
    char **p;
    char *pp;
    char *str;
    ...
    if (NULL == argv || NULL == argv[0] || (int) start > cci_argv_count(argv)) {
	return strdup("");
    }
    /* Find the total string length in argv including delimiters.  The
       last delimiter is replaced by the NULL character. */
...
char **cci_argv_copy(char **argv)
{
    ...
    while (NULL != *argv) {
	if (CCI_SUCCESS != cci_argv_append(&dupc, &dupv, *argv)) {
	    cci_argv_free(dupv);
	    return NULL;
	}
	++argv;
...
int cci_argv_delete(int *argc, char ***argv, int start, int num_to_delete)
{
    int i;
    int count;
    int suffix_count;
    ...
    count = cci_argv_count(*argv);
    if (start > count) {
	return CCI_SUCCESS;
    } else if (start < 0 || num_to_delete < 0) {
	return CCI_EINVAL;
    }
    /* Ok, we have some tokens to delete.  Calculate the new length of the
       argv array. */
...
int cci_argv_insert(char ***target, int start, char **source)
{
    ...
    target_count = cci_argv_count(*target);
    source_count = cci_argv_count(source);
    if (start > target_count) {
	for (i = 0; i < source_count; ++i) {
	    cci_argv_append(&target_count, target, source[i]);
	}
    }
