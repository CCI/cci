				   ...
				   int include_empty)
{
	...
	while (src_string && *src_string) {
		...
		/* tail argument, add straight from the original string */
			...
			if (CCI_SUCCESS !=
			    cci_argv_append(&argc, &argv, src_string))
				return NULL;
			src_string = p;
			continue;
		}
		/* long argument, malloc buffer, copy and add */
			...
			if (CCI_SUCCESS !=
			    cci_argv_append(&argc, &argv, argtemp)) {
				free(argtemp);
				return NULL;
			}
			free(argtemp);
