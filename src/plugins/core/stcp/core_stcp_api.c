...
volatile int stcp_shut_down = 0;
stcp_globals_t *sglobals = NULL;
 ...
 * Local functions
 */
static int stcp_init(cci_plugin_core_t *plugin, uint32_t abi_ver, uint32_t flags, uint32_t * caps);
static int stcp_finalize(cci_plugin_core_t * plugin);
static const char *stcp_strerror(cci_endpoint_t * endpoint,
				 enum cci_status status);
static int stcp_create_endpoint(cci_device_t * device,
				int flags,
				cci_endpoint_t ** endpoint,
				cci_os_handle_t * fd);
static int stcp_destroy_endpoint(cci_endpoint_t * endpoint);
static int stcp_accept(cci_event_t *event, const void *context);
static int stcp_reject(cci_event_t *conn_req);
static int stcp_connect(cci_endpoint_t * endpoint, const char *server_uri,
			const void *data_ptr, uint32_t data_len,
			cci_conn_attribute_t attribute,
			const void *context, int flags, const struct timeval *timeout);
static int stcp_disconnect(cci_connection_t * connection);
static int stcp_set_opt(cci_opt_handle_t * handle,
			cci_opt_level_t level,
			cci_opt_name_t name, const void *val, int len);
static int stcp_get_opt(cci_opt_handle_t * handle,
			cci_opt_level_t level,
			cci_opt_name_t name, void **val, int *len);
static int stcp_arm_os_handle(cci_endpoint_t * endpoint, int flags);
static int stcp_get_event(cci_endpoint_t * endpoint,
			  cci_event_t ** const event);
static int stcp_return_event(cci_event_t * event);
static int stcp_send(cci_connection_t * connection,
		     const void *msg_ptr, uint32_t msg_len, const void *context, int flags);
static int stcp_sendv(cci_connection_t * connection,
		      const struct iovec *data, uint32_t iovcnt,
		      const void *context, int flags);
static int stcp_rma_register(cci_endpoint_t * endpoint,
			     void *start, uint64_t length,
			     int flags, uint64_t * rma_handle);
static int stcp_rma_deregister(cci_endpoint_t * endpoint, uint64_t rma_handle);
static int stcp_rma(cci_connection_t * connection,
		    const void *header_ptr, uint32_t header_len,
		    uint64_t local_handle, uint64_t local_offset,
		    uint64_t remote_handle, uint64_t remote_offset,
		    uint64_t data_len, const void *context, int flags);

static uint8_t stcp_ip_hash(in_addr_t ip, uint16_t port);
static void stcp_progress_sends(cci__dev_t * dev);
static void *stcp_progress_thread(void *arg);
static void *stcp_recv_thread(void *arg);
static inline void stcp_progress_dev(cci__dev_t * dev);
static int stcp_sendto(cci_os_handle_t sock, void *buf, int len,
 ...
 * plugin resides.
 */
cci_plugin_core_t cci_core_stcp_plugin = {
	{
	 /* Logistics */
	 CCI_ABI_VERSION,
	 ...
	 cci_core_stcp_post_load,
	 cci_core_stcp_pre_unload,
	 },
	...
	stcp_finalize,
	stcp_strerror,
	stcp_create_endpoint,
	stcp_destroy_endpoint,
	stcp_accept,
	stcp_reject,
	stcp_connect,
	stcp_disconnect,
...
static inline void
stcp_sin_to_name(struct sockaddr_in sin, char *buffer, int len)
{
...
static inline const char *stcp_msg_type(stcp_msg_type_t type)
{
	switch (type) {
	case STCP_MSG_CONN_REQUEST:
		return "conn_request";
	case STCP_MSG_CONN_REPLY:
		return "conn_reply";
	case STCP_MSG_CONN_ACK:
		return "conn_ack";
	case STCP_MSG_DISCONNECT:
		return "disconnect";
	case STCP_MSG_SEND:
		return "send";
	case STCP_MSG_RNR:
		return "receiver not ready";
	case STCP_MSG_KEEPALIVE:
		return "keepalive";
	case STCP_MSG_PING:
		return "ping for RTTM";
	case STCP_MSG_ACK_ONLY:
		return "ack_only";
	case STCP_MSG_ACK_UP_TO:
		return "ack_up_to";
	case STCP_MSG_SACK:
		return "selective ack";
	case STCP_MSG_RMA_WRITE:
		return "RMA write";
	case STCP_MSG_RMA_READ_REQUEST:
		return "RMA read request";
	case STCP_MSG_RMA_READ_REPLY:
		return "RMA read reply";
	case STCP_MSG_RMA_INVALID:
		return "invalid RMA handle";
	case STCP_MSG_INVALID:
		assert(0);
		return "invalid";
	case STCP_MSG_TYPE_MAX:
		assert(0);
		return "type_max";
	}
	return NULL;
...
static int stcp_init(cci_plugin_core_t *plugin,
		     uint32_t abi_ver, uint32_t flags, uint32_t * caps)
{
	...
	srandom((unsigned int)stcp_get_usecs());
	...
	if (!globals->configfile) {
		...
		for (i = 0; i < sglobals->ifa_count; i++) {
			...
			device = &dev->device;
			device->max_send_size = STCP_DEFAULT_MSS;
...
static inline int
stcp_set_nonblocking(cci__ep_t *ep, cci__conn_t *conn)
{
	int ret, flags;
	stcp_ep_t *sep = ep->priv;
	stcp_conn_t *sconn = conn->priv;
...
static inline void stcp_close_socket(cci__ep_t *ep, cci__conn_t *conn)
{
	stcp_ep_t *sep = ep->priv;
	stcp_conn_t *sconn = conn->priv;
	...
	if (sconn->sock == sep->nfds - 1) {
		...
		stcp_fd_idx_t *idx;
		for (i = sock - 1; i >= 0; i--) {
			idx = (stcp_fd_idx_t *) & sglobals->fd_idx[i];
			if (idx->type != STCP_FD_UNUSED) {
				sglobals->nfds = i + 1;
				found = 1;
				break;
			}
				...
				cci_os_handle_t * fd)
{
	...
	endpoint->max_recv_buffer_count = STCP_EP_RX_CNT;
	ep->rx_buf_cnt = STCP_EP_RX_CNT;
	ep->tx_buf_cnt = STCP_EP_TX_CNT;
	ep->buffer_len = dev->device.max_send_size + STCP_MAX_HDRS;
	ep->tx_timeout = STCP_EP_TX_TIMEOUT_SEC * 1000000;

	sep = ep->priv;
	sep->ids = calloc(STCP_NUM_BLOCKS, sizeof(*sep->ids));
	if (!sep->ids) {
		ret = CCI_ENOMEM;
		goto out;
	}
	...
	sprintf(name, "ip://");
	stcp_sin_to_name(sep->sin, name + (uintptr_t) 5, sizeof(name) - 5);
	endpoint->name = strdup(name);

	for (i = 0; i < STCP_EP_HASH_SIZE; i++) {
	...
	ret = stcp_set_nonblocking(sep->sock, STCP_FD_EP, ep);
	if (ret)
		goto out;
...
static int stcp_destroy_endpoint(cci_endpoint_t * endpoint)
{
	...
	if (sep) {
		...
		while (!TAILQ_EMPTY(&sep->txs)) {
			...
			tx = TAILQ_FIRST(&sep->txs);
			TAILQ_REMOVE(&sep->txs, tx, tentry);
			if (tx->state == STCP_TX_QUEUED)
				TAILQ_REMOVE(&sdev->queued, tx, dentry);
			else if (tx->state == STCP_TX_PENDING)
		...
		while (!TAILQ_EMPTY(&sep->rma_ops)) {
			stcp_rma_op_t *rma_op = TAILQ_FIRST(&sep->rma_ops);
			TAILQ_REMOVE(&sep->rma_ops, rma_op, entry);
			free(rma_op);
		}
		while (!TAILQ_EMPTY(&sep->handles)) {
			stcp_rma_handle_t *handle = TAILQ_FIRST(&sep->handles);
...
static void stcp_get_id(stcp_ep_t * ep, uint32_t * id)
{
	uint32_t n, block, offset;
	uint64_t *b;

	while (1) {
		n = random() % STCP_NUM_BLOCKS;
		block = n / STCP_BLOCK_SIZE;
		offset = n % STCP_BLOCK_SIZE;
		b = &ep->ids[block];

		if ((*b & (1ULL << offset)) == 0) {
			*b |= (1ULL << offset);
			*id = (block * STCP_BLOCK_SIZE) + offset;
			break;
		}
	}
	return;
...
static void stcp_put_id(stcp_ep_t * ep, uint32_t id)
{
	uint32_t block, offset;
	uint64_t *b;

	block = id / STCP_BLOCK_SIZE;
	offset = id % STCP_BLOCK_SIZE;
	b = &ep->ids[block];
...
static inline uint32_t stcp_get_new_seq(void)
{
	return ((uint32_t) random() & STCP_SEQ_MASK);
}
...
static int stcp_accept(cci_event_t *event, const void *context)
{
	...
	cci__dev_t *dev = NULL;
	stcp_ep_t *sep = NULL;
	stcp_conn_t *sconn = NULL;
	stcp_dev_t *sdev = NULL;
	stcp_header_r_t *hdr_r = NULL;
	stcp_msg_type_t type;
	stcp_tx_t *tx = NULL;
	stcp_rx_t *rx = NULL;
	stcp_handshake_t *hs = NULL;
	...
	rx = container_of(evt, stcp_rx_t, evt);
	ep = evt->ep;
	...
	hdr_r = rx->buffer;
	stcp_parse_header(&hdr_r->header, &type, &a, &b, &unused);
	stcp_parse_seq_ts(&hdr_r->seq_ts, &peer_seq, &peer_ts);
	...
	hs = (stcp_handshake_t *) (rx->buffer +
				   (uintptr_t) sizeof(stcp_header_r_t));
	stcp_parse_handshake(hs, &id, &ack, &max_recv_buffer_count, &mss, &ka);
	if (ka != 0UL) {
	...
	sconn->conn = conn;
	sconn->cwnd = STCP_INITIAL_CWND;
	sconn->status = STCP_CONN_READY;	/* set ready since the app thinks it is */
	*((struct sockaddr_in *)&sconn->sin) = rx->sin;
	sconn->peer_id = id;
	stcp_get_id(sep, &sconn->id);
	sconn->seq = stcp_get_new_seq();	/* even for UU since this reply is reliable */
	...
	if (cci_conn_is_reliable(conn)) {
		...
		sconn->last_ack_ts = stcp_get_usecs();
		sconn->ssthresh = sconn->max_tx_cnt;
	}

	/* insert in sock ep's list of conns */

	i = stcp_ip_hash(sconn->sin.sin_addr.s_addr, sconn->sin.sin_port);
	pthread_mutex_lock(&ep->lock);
	...
	tx->msg_type = STCP_MSG_CONN_REPLY;
	...
	hdr_r = (stcp_header_r_t *) tx->buffer;
	stcp_pack_conn_reply(&hdr_r->header, CCI_SUCCESS /* FIXME */ ,
			     sconn->peer_id);
	stcp_pack_seq_ts(&hdr_r->seq_ts, sconn->seq,
			 (uint32_t) sconn->last_ack_ts);
	hs = (stcp_handshake_t *) (tx->buffer + sizeof(*hdr_r));
	stcp_pack_handshake(hs, sconn->id, peer_seq,
 ...
 */
static int stcp_reject(cci_event_t *event)
{
	int ret = CCI_SUCCESS;
	uint8_t a;
	uint16_t b;
	...
	stcp_parse_header(&hdr_r->header, &type, &a, &b, &peer_id);
	stcp_parse_seq_ts(&hdr_r->seq_ts, &peer_seq, &peer_ts);
	...
	hdr_r = (stcp_header_r_t *) tx->buffer;
	stcp_pack_conn_reply(&hdr_r->header, CCI_ECONNREFUSED /* FIXME */ ,
			     peer_id);
	stcp_pack_seq_ts(&hdr_r->seq_ts, peer_seq, 0);
	...
	memset(name, 0, sizeof(name));
	stcp_sin_to_name(rx->sin, name, sizeof(name));
...
static int stcp_getaddrinfo(const char *uri, in_addr_t * in, uint16_t * port)
{
	int ret;
...
static stcp_conn_t *stcp_find_open_conn(stcp_ep_t * sep, in_addr_t ip,
					uint16_t port, uint32_t id)
{
	uint8_t i;
	struct s_conns *conn_list;
	stcp_conn_t *sconn = NULL, *sc;

	CCI_ENTER;

	i = stcp_ip_hash(ip, port);
...
static stcp_conn_t *stcp_find_active_conn(stcp_ep_t * sep, in_addr_t ip,
					  uint32_t id)
{
	uint8_t i;
...
static stcp_conn_t *stcp_find_conn(stcp_ep_t * sep, in_addr_t ip, uint16_t port,
				   uint32_t id, stcp_msg_type_t type)
{
	switch (type) {
	case STCP_MSG_CONN_REPLY:
		return stcp_find_active_conn(sep, ip, id);
	default:
		return stcp_find_open_conn(sep, ip, port, id);
	}
}
			...
			const void *context, int flags, const struct timeval *timeout)
{
	...
	sconn->status = STCP_CONN_ACTIVE;
	sconn->cwnd = STCP_INITIAL_CWND;
	sin = (struct sockaddr_in *)&sconn->sin;
	memset(sin, 0, sizeof(*sin));
	sin->sin_family = AF_INET;

	ret = stcp_getaddrinfo(server_uri, &ip, &port);
	if (ret)
		goto out;
	...
	/* prep the tx */
	tx->msg_type = STCP_MSG_CONN_REQUEST;
	...
	if (0)
		stcp_put_id(sep, 0);
	stcp_pack_conn_request(&hdr_r->header, attribute,
			       (uint16_t) data_len, 0);
	tx->len = sizeof(*hdr_r);

	/* add seq and ack */

	sconn->seq = stcp_get_new_seq();
	sconn->seq_pending = sconn->seq - 1;
	sconn->last_ack_seq = sconn->seq;
	tx->seq = sconn->seq;
	stcp_pack_seq_ts(&hdr_r->seq_ts, tx->seq, ts);

	/* add handshake */
	hs = (stcp_handshake_t *) & hdr_r->data;
	if (keepalive != 0UL)
		conn->keepalive_timeout = keepalive;
	stcp_pack_handshake(hs, sconn->id, 0,
...
static int stcp_get_event(cci_endpoint_t * endpoint, cci_event_t ** const event)
{
	int ret = CCI_SUCCESS;
	cci__ep_t *ep;
	...
	TAILQ_FOREACH(e, &ep->evts, entry) {
		if (e->event.type == CCI_EVENT_SEND) {
			/* NOTE: if it is blocking, skip it since stcp_sendv()
			 * is waiting on it
			 */
			stcp_tx_t *tx = container_of(e, stcp_tx_t, evt);
...
static int stcp_return_event(cci_event_t * event)
{
	...
	switch (event->type) {
	case CCI_EVENT_SEND:
		tx = container_of(evt, stcp_tx_t, evt);
		pthread_mutex_lock(&ep->lock);
...
static int stcp_sendmsg(cci_os_handle_t sock, void **ptrs, uint32_t * lens,
		       ...
		       const struct sockaddr_in sin)
{
	...
	if (0)
		stcp_sendmsg(sock, NULL, NULL, 0, sin);
...
static void stcp_progress_pending(cci__dev_t * dev)
{
	int ret;
	uint64_t now;
	stcp_tx_t *tx, *tmp;
	...
	TAILQ_HEAD(s_idle_txs, stcp_tx) idle_txs =
	...
	TAILQ_FOREACH_SAFE(tx, &sdev->pending, dentry, tmp) {
		...
		/* has it timed out? */
		if (STCP_U64_LT(tx->timeout_us, now)) {
			/* dequeue */
			...
			case STCP_MSG_CONN_REQUEST:
				{
					...
					if (conn->uri)
						free((char *)conn->uri);
					sconn->status = STCP_CONN_CLOSING;
					i = stcp_ip_hash(sconn->sin.
							 sin_addr.s_addr, 0);
					active_list = &sep->active_hash[i];
					pthread_mutex_lock(&ep->lock);
			...
			if (tx->flags & CCI_FLAG_SILENT &&
			    (tx->msg_type == STCP_MSG_SEND ||
			     tx->msg_type == STCP_MSG_RMA_WRITE)) {

				tx->state = STCP_TX_IDLE;
				/* store locally until we can drop the dev->lock */
				TAILQ_INSERT_HEAD(&idle_txs, tx, dentry);
			} else {
				tx->state = STCP_TX_COMPLETED;
				/* store locally until we can drop the dev->lock */
		...
		if ((tx->last_attempt_us +
		     ((1 << tx->send_count) * STCP_RESEND_TIME_SEC * 1000000)) >
		    now)
			continue;
...
#if 0
		if (tx->send_count == 1 && tx->msg_type == STCP_MSG_SEND && 0) {
		...
		debug(CCI_DB_MSG, "re-sending %s msg seq %u count %u",
		      stcp_msg_type(tx->msg_type), tx->seq, tx->send_count);
		ret = stcp_sendto(sep->sock, tx->buffer, tx->len, sconn->sin);
		if (ret != tx->len) {
...
static void stcp_progress_queued(cci__dev_t * dev)
{
	int ret, is_reliable;
	uint32_t timeout;
	uint64_t now;
	...
	TAILQ_FOREACH_SAFE(tx, &sdev->queued, dentry, tmp) {
		...
		if (STCP_U64_LT(tx->timeout_us, now)) {
			...
			default:
				...
				debug(CCI_DB_WARN, "%s: timeout of %s msg",
				      __func__, stcp_msg_type(tx->msg_type));
				CCI_EXIT;
				return;
			}
			TAILQ_REMOVE(&sdev->queued, tx, dentry);
		...
		if ((tx->last_attempt_us + (STCP_RESEND_TIME_SEC * 1000000)) >
		    now)
			continue;

#if 0
		if (sconn->pending > sconn->cwnd &&
		    tx->msg_type == STCP_MSG_SEND && 0) {
			continue;
		}
#endif
		...
		debug(CCI_DB_MSG, "sending %s msg seq %u",
		      stcp_msg_type(tx->msg_type), tx->seq);
		ret = stcp_sendto(sep->sock, tx->buffer, tx->len, sconn->sin);
		if (ret == -1) {
			switch (errno) {
			default:
...
static void stcp_progress_sends(cci__dev_t * dev)
{
	stcp_progress_pending(dev);
	stcp_progress_queued(dev);
		     ...
		     const void *msg_ptr, uint32_t msg_len, const void *context, int flags)
{
	...
	return stcp_sendv(connection, &iov, iovcnt, context, flags);
}
		      ...
		      const void *context, int flags)
{
	...
	if (!is_reliable) {
		...
		tx->last_attempt_us = 0ULL;
		tx->timeout_us =
		    stcp_get_usecs() + STCP_EP_TX_TIMEOUT_SEC * 1000000;
	}
	...
	/* pack buffer */
	hdr = (stcp_header_t *) tx->buffer;
	stcp_pack_send(hdr, data_len, sconn->peer_id);
	tx->len = sizeof(*hdr);

	/* if reliable, add seq and ack */

	if (is_reliable) {
		stcp_header_r_t *hdr_r = tx->buffer;
		uint32_t ts = 0;
	...
	if (!is_reliable) {
		...
		if (ret == tx->len) {
			...
			debug(CCI_DB_MSG, "sent UU msg with %d bytes",
			      tx->len - (int)sizeof(stcp_header_t));
	...
	if (tx->flags & CCI_FLAG_BLOCKING) {
		struct timeval tv = { 0, STCP_PROG_TIME_US / 2 };

		while (tx->state != STCP_TX_COMPLETED)
...
{
	int ret = CCI_EINVAL;
	stcp_rma_handle_t *handle =
	    (stcp_rma_handle_t *) ((uintptr_t) rma_handle);
	cci__ep_t *ep = NULL;
	stcp_ep_t *sep = NULL;
	stcp_rma_handle_t *h = NULL;
	stcp_rma_handle_t *tmp = NULL;
...
static inline void
generate_context_id(stcp_conn_t * sconn, const void *context, uint64_t * context_id)
{
	uint64_t index = 0;
		    ...
		    uint64_t data_len, const void *context, int flags)
{
	...
	stcp_dev_t *sdev = NULL;
	stcp_conn_t *sconn = NULL;
	stcp_rma_handle_t *local =
	    (stcp_rma_handle_t *) ((uintptr_t) local_handle);
	stcp_rma_handle_t *h = NULL;
	stcp_rma_op_t *rma_op = NULL;
	...
	if (flags & CCI_FLAG_WRITE) {
		...
		cnt = rma_op->num_msgs < STCP_RMA_DEPTH ?
		    rma_op->num_msgs : STCP_RMA_DEPTH;
		...
		for (i = 0; i < cnt; i++) {
			...
			stcp_rma_header_t *write =
			    (stcp_rma_header_t *) tx->buffer;
			...
			stcp_pack_rma_write(write, tx->len, sconn->peer_id,
			...
			tx->len += sizeof(stcp_rma_header_t);
		}
		pthread_mutex_lock(&dev->lock);
		pthread_mutex_lock(&ep->lock);
		for (i = 0; i < cnt; i++)
		...
		read = tx->buffer;
		sep = ep->priv;
		memset(tx->buffer, 0, sizeof(stcp_rma_header_t));
		tx->seq = ++(sconn->seq);
		tx->timeout_us = 0ULL;
		tx->last_attempt_us = 0ULL;
		stcp_pack_rma_read(read, data_len, sconn->peer_id, tx->seq, 0,
 ...
 */
static inline void stcp_handle_seq(stcp_conn_t * sconn, uint32_t seq)
{
	int done = 0;
	stcp_ack_t *ack = NULL;
	stcp_ack_t *last = NULL;
	stcp_ack_t *tmp = NULL;
	...
	if (STCP_SEQ_LTE(seq, sconn->acked)) {
	...
	TAILQ_FOREACH_SAFE(ack, &sconn->acks, entry, tmp) {
		if (STCP_SEQ_GTE(seq, ack->start) &&
		    STCP_SEQ_LTE(seq, ack->end)) {
			...
			stcp_ack_t *next = TAILQ_NEXT(ack, entry);
			...
			break;
		} else if (last && STCP_SEQ_GT(seq, last->end) &&
			   STCP_SEQ_LT(seq, ack->start)) {
			stcp_ack_t *new;
...
static void
stcp_handle_active_message(stcp_conn_t * sconn,
			   stcp_rx_t * rx, uint16_t len, uint32_t id)
{
	cci__evt_t *evt;
	cci__conn_t *conn = sconn->conn;
	stcp_header_t *hdr;	/* wire header */
	union cci_event *event;	/* generic CCI event */
	cci_endpoint_t *endpoint;	/* generic CCI endpoint */
	cci__ep_t *ep;
	...
	if (cci_conn_is_reliable(conn)) {
		stcp_header_r_t *hdr_r = (stcp_header_r_t *) rx->buffer;
 ...
 */
static void stcp_handle_rnr(stcp_conn_t * sconn, uint32_t seq, uint32_t ts)
{
	stcp_tx_t *tx = NULL;
	stcp_tx_t *tmp = NULL;
 ...
 */
static void
stcp_handle_ack(stcp_conn_t * sconn,
		stcp_msg_type_t type, stcp_rx_t * rx, int count, uint32_t id)
{
	int i = 0;
	int found = 0;
	cci__conn_t *conn = sconn->conn;
	...
	stcp_tx_t *tx = NULL;
	stcp_tx_t *tmp = NULL;
	stcp_header_r_t *hdr_r = rx->buffer;
	uint32_t acks[STCP_MAX_SACK * 2];
	...
	TAILQ_INIT(&idle_txs);
	TAILQ_INIT(&evts);
	TAILQ_HEAD(s_queued, stcp_tx) queued = TAILQ_HEAD_INITIALIZER(queued);
	TAILQ_INIT(&queued);
	...
	stcp_parse_ack(hdr_r, type, acks, count);

	if (type == STCP_MSG_ACK_ONLY) {
		if (sconn->seq_pending == acks[0] - 1)
			sconn->seq_pending = acks[0];
	} else if (type == STCP_MSG_ACK_UP_TO) {
		sconn->seq_pending = acks[0];
	}
	...
	TAILQ_FOREACH_SAFE(tx, &sconn->tx_seqs, tx_seq, tmp) {
		if (type == STCP_MSG_ACK_ONLY) {
			...
			for (i = 0; i < count; i += 2) {
				if (STCP_SEQ_GTE(tx->seq, acks[i]) &&
				    STCP_SEQ_LTE(tx->seq, acks[i + 1])) {
	...
	while (!TAILQ_EMPTY(&idle_txs)) {
		...
		if (rma_op && rma_op->status == CCI_SUCCESS) {
			stcp_rma_handle_t *local =
			    (stcp_rma_handle_t *) ((uintptr_t) rma_op->
						   local_handle);
			rma_op->completed++;
			...
			if (rma_op->next < rma_op->num_msgs) {
				...
				stcp_pack_rma_write(write, tx->len,
				...
				if (rma_op->msg_len) {
					...
					tx->evt.ep = ep;
					memset(tx->buffer, 0,
					       sizeof(stcp_rma_header_t));
					write =
					    (stcp_rma_header_t *) tx->buffer;
					context_id = (uint64_t) rma_op->context;
					stcp_pack_rma_write_done(write,
								 sconn->peer_id,
								 tx->seq, 0);
...
static void
stcp_handle_conn_request(stcp_rx_t * rx,
			 ...
			 uint16_t len, struct sockaddr_in sin, cci__ep_t * ep)
{
	...
	memset(name, 0, sizeof(name));
	stcp_sin_to_name(sin, name, sizeof(name));
	...
	*((uint32_t *) & rx->evt.event.request.data_len) = len;
	if (len)
		*((void **)&rx->evt.event.request.data_ptr) =
		    (void *)((((stcp_header_r_t *) rx->buffer)->data) +
			     (uintptr_t) sizeof(stcp_handshake_t));
	else
 ...
 * Ready conn   Error
 */
static void stcp_handle_conn_reply(stcp_conn_t * sconn,	/* NULL if rejected */
				   stcp_rx_t * rx, uint8_t reply,	/* CCI_SUCCESS or CCI_ECONNREFUSED */
				   uint16_t unused,
				   uint32_t id,
				   struct sockaddr_in sin, cci__ep_t * ep)
{
	...
	stcp_ep_t *sep = NULL;
	stcp_dev_t *sdev = NULL;
	stcp_tx_t *tx = NULL, *tmp = NULL, *t = NULL;
	stcp_header_r_t *hdr_r;	/* wire header */
	union cci_event *event;	/* generic CCI event */
	...
	if (!sconn) {
		...
		sconn =
		    stcp_find_conn(sep, sin.sin_addr.s_addr, sin.sin_port, id,
				   STCP_MSG_SEND);
		if (!sconn) {
			stcp_header_r_t hdr;
			...
			stcp_sin_to_name(sin, from, sizeof(from));
			debug((CCI_DB_CONN | CCI_DB_MSG),
			...
			stcp_pack_conn_ack(&hdr.header, sconn->peer_id);
			ret = stcp_sendto(sep->sock, &hdr, len, sin);
			if (ret != len) {
				debug((CCI_DB_CONN | CCI_DB_MSG),
	...
	hdr_r = (stcp_header_r_t *) rx->buffer;

	/* TODO handle ack */

	stcp_parse_seq_ts(&hdr_r->seq_ts, &seq, &ts);	//FIXME do something with ts

	if (sconn->status == STCP_CONN_ACTIVE) {
		uint32_t peer_id, ack, max_recv_buffer_count, mss, keepalive;
		struct s_active *active_list;

		debug(CCI_DB_CONN, "transition active connection to ready");

		hs = (stcp_handshake_t *) (rx->buffer + sizeof(*hdr_r));
		/* With conn_reply, we do not care about the keepalive param */
		stcp_parse_handshake(hs, &peer_id, &ack, &max_recv_buffer_count,
				     &mss, &keepalive);
		...
		i = stcp_ip_hash(sin.sin_addr.s_addr, 0);
		active_list = &sep->active_hash[i];
		pthread_mutex_lock(&ep->lock);
		TAILQ_REMOVE(active_list, sconn, entry);
		pthread_mutex_unlock(&ep->lock);
		...
		if (CCI_SUCCESS == reply) {
			...
			sconn->status = STCP_CONN_READY;
			*((struct sockaddr_in *)&sconn->sin) = sin;
			sconn->acked = seq;

			i = stcp_ip_hash(sin.sin_addr.s_addr, sin.sin_port);
			pthread_mutex_lock(&ep->lock);
			TAILQ_INSERT_TAIL(&sep->conn_hash[i], sconn, entry);
			pthread_mutex_unlock(&ep->lock);
	...
	} else if (sconn->status == STCP_CONN_READY) {
		pthread_mutex_lock(&ep->lock);
		if (!TAILQ_EMPTY(&sep->idle_txs)) {
			tx = TAILQ_FIRST(&sep->idle_txs);
			TAILQ_REMOVE(&sep->idle_txs, tx, dentry);
		...
		if (!tx) {
			...
			stcp_sin_to_name(sin, to, sizeof(to));
	...
	hdr_r = tx->buffer;
	stcp_pack_conn_ack(&hdr_r->header, sconn->peer_id);
	sconn->last_ack_ts = stcp_get_usecs();
	/* the conn_ack acks the server's seq in the timestamp */
	stcp_pack_seq_ts(&hdr_r->seq_ts, tx->seq, seq);
 ...
 */
static void
stcp_handle_conn_ack(stcp_conn_t * sconn,
		     stcp_rx_t * rx,
		     uint8_t unused1,
		     uint16_t unused2, uint32_t peer_id, struct sockaddr_in sin)
{
	...
	hdr_r = rx->buffer;
	stcp_parse_seq_ts(&hdr_r->seq_ts, &seq, &ts);
 ...
 */
static void
stcp_handle_rma_read_request(stcp_conn_t * sconn, stcp_rx_t * rx,
			     uint16_t data_len)
{
	cci__ep_t *ep = NULL;
	cci__conn_t *conn = sconn->conn;
	stcp_rma_header_t *read = rx->buffer;
	...
	stcp_parse_rma_handle_offset(&read->local,
				     &msg_local_handle, &msg_local_offset);
	stcp_parse_rma_handle_offset(&read->remote,
				     &msg_remote_handle, &msg_remote_offset);

	stcp_parse_seq_ts(&hdr_r->seq_ts, &seq, &ts);
	stcp_handle_seq(sconn, seq);
	//remote = (stcp_rma_handle_t *) (uintptr_t) remote_handle;
...
static void
stcp_handle_rma_write(stcp_conn_t * sconn, stcp_rx_t * rx, uint16_t len)
{
	cci__ep_t *ep = NULL;
	cci__conn_t *conn = sconn->conn;
	stcp_ep_t *sep = NULL;
	stcp_rma_header_t *write = rx->buffer;
	uint64_t local_handle;
	uint64_t local_offset;
	uint64_t remote_handle;	/* our handle */
	uint64_t remote_offset;	/* our offset */
	stcp_rma_handle_t *remote, *h;

	ep = container_of(conn->connection.endpoint, cci__ep_t, endpoint);
	sep = ep->priv;

	stcp_parse_rma_handle_offset(&write->local, &local_handle,
				     &local_offset);
	stcp_parse_rma_handle_offset(&write->remote, &remote_handle,
				     &remote_offset);
	remote = (stcp_rma_handle_t *) (uintptr_t) remote_handle;
...
static inline void
lookup_contextid(stcp_conn_t * sconn, uint64_t context_id, const void **context)
{
...
static void
stcp_handle_rma_write_done(stcp_conn_t * sconn, stcp_rx_t * rx, uint16_t len)
{
	cci__evt_t *evt;
	cci__conn_t *conn = sconn->conn;
	union cci_event *event;	/* generic CCI event */
	...
	//stcp_rma_header_t *rma_hdr = rx->buffer;
	const void *context;
	stcp_header_r_t *hdr_r = rx->buffer;
...
static inline void stcp_drop_msg(cci_os_handle_t sock)
{
	char buf[4];
	struct sockaddr sa;
...
static int stcp_recvfrom_ep(cci__ep_t * ep)
{
...
#endif
	if (!rx) {
		char tmp_buff[STCP_UDP_MAX];
		stcp_header_t *hdr = NULL;
		...
		ret = recvfrom(sep->sock, (void *)tmp_buff, STCP_UDP_MAX,
			       0, (struct sockaddr *)&sin, &sin_len);
		if (ret < (int)sizeof(stcp_header_t)) {
			debug(CCI_DB_INFO,
		...
		hdr = (stcp_header_t *) tmp_buff;
		stcp_parse_header(hdr, &type, &a, &b, &id);
		sconn =
		    stcp_find_conn(sep, sin.sin_addr.s_addr, sin.sin_port, id,
				   type);
		conn = sconn->conn;
		...
		if (cci_conn_is_reliable(sconn->conn)) {
			...
			header_r = (stcp_header_r_t *) tmp_buff;
			stcp_parse_seq_ts(&header_r->seq_ts, &seq, &ts);
			sconn->rnr = seq;
			drop_msg = 1;
			goto out;
		} else {
	...
	stcp_parse_header(rx->buffer, &type, &a, &b, &id);
	if (STCP_MSG_CONN_REPLY == type) {
		reply = 1;
	} else if (STCP_MSG_CONN_REQUEST == type) {
		request = 1;
		rx->sin = sin;
	}

	if (STCP_MSG_KEEPALIVE == type)
		ka = 1;
...

	{
		...
		if (CCI_DB_MSG & cci__debug) {
			...
			debug((CCI_DB_MSG),
			      ...
			      sep->sock, stcp_msg_type(type), name, a + b);
		}
	}
	...
	if (!ka && !sconn && !reply && !request) {
		debug((CCI_DB_CONN | CCI_DB_MSG),
		      ...
		      stcp_msg_type(type), inet_ntoa(sin.sin_addr),
		      ntohs(sin.sin_port));
		q_rx = 1;
		goto out;
	}
	...
	switch (type) {
	case STCP_MSG_CONN_REQUEST:
		stcp_handle_conn_request(rx, a, b, sin, ep);
		break;
	case STCP_MSG_CONN_REPLY:
		stcp_handle_conn_reply(sconn, rx, a, b, id, sin, ep);
		break;
	case STCP_MSG_CONN_ACK:
		stcp_handle_conn_ack(sconn, rx, a, b, id, sin);
		break;
	case STCP_MSG_DISCONNECT:
		break;
	case STCP_MSG_SEND:
		stcp_handle_active_message(sconn, rx, b, id);
		break;
	case STCP_MSG_RNR:{
			stcp_header_r_t *hdr_r = rx->buffer;

			stcp_parse_seq_ts(&hdr_r->seq_ts, &seq, &ts);
			stcp_handle_rnr(sconn, seq, ts);
			break;
		}
	case STCP_MSG_KEEPALIVE:
		/* Nothing to do? */
	...
	case STCP_MSG_ACK_UP_TO:
	case STCP_MSG_SACK:
		stcp_handle_ack(sconn, type, rx, (int)a, id);
		break;
	case STCP_MSG_RMA_WRITE:
		stcp_handle_rma_write(sconn, rx, b);
		break;
	case STCP_MSG_RMA_WRITE_DONE:
		stcp_handle_rma_write_done(sconn, rx, b);
		break;
	case STCP_MSG_RMA_READ_REQUEST:
		stcp_handle_rma_read_request(sconn, rx, b);
		break;
	case STCP_MSG_RMA_READ_REPLY:
		break;
	default:
		debug(CCI_DB_MSG, "unknown active message with type %u",
		      (enum stcp_msg_type)type);
	}
	...
	if (drop_msg) {
		if (cci_conn_is_reliable(sconn->conn) && sconn->rnr == seq) {
			...
			hdr_r = (stcp_header_r_t *) buffer;
			stcp_pack_nack(hdr_r,
				       STCP_MSG_RNR,
			...
			stcp_sendto(sep->sock, buffer, len, sconn->sin);
		}

		/* Drop the message */
		stcp_drop_msg(sep->sock);
	}
 ...
 */
static void stcp_keepalive(void)
{
	stcp_conn_t *sconn;
	cci__conn_t *conn;
	uint64_t now = 0ULL;
	...
	TAILQ_FOREACH(sconn, &sglobals->ka_conns, entry) {
		...
		if (STCP_U64_LT(now, ka_timeout)) {
			...
			sep = ep->priv;
			memset(buffer, 0, sizeof(buffer));
			hdr = (stcp_header_t *) buffer;
			stcp_pack_keepalive(hdr, sconn->peer_id);
			len = sizeof(*hdr);
			stcp_sendto(sep->sock, buffer, len, sconn->sin);
		}
	}
...
static void stcp_ack_conns(cci__ep_t * ep)
{
	int i;
	cci__dev_t *dev = ep->dev;
	stcp_dev_t *sdev = dev->priv;
	...
	TAILQ_HEAD(s_txs, stcp_tx) txs = TAILQ_HEAD_INITIALIZER(txs);
	TAILQ_INIT(&txs);
			...
			TAILQ_FOREACH(sconn, &sep->conn_hash[i], entry) {
				if (!TAILQ_EMPTY(&sconn->acks)) {
					...
					len =
					    sizeof(*hdr_r) +
					    (count * sizeof(acks[0]));
					stcp_sendto(sep->sock, buffer, len,
						    sconn->sin);
				}
			}
		}
...
static void *stcp_progress_thread(void *arg)
{
	struct timeval tv = { 0, STCP_PROG_TIME_US };

	assert(!arg);
	pthread_mutex_lock(&sglobals->lock);
	while (!stcp_shut_down) {
		cci__dev_t *dev;
		cci_device_t const **device;
...
static void *stcp_recv_thread(void *arg)
{
	...
	while (!stcp_shut_down) {
		...
		FD_ZERO(&fds);
		for (i = 0; i < nfds; i++) {
			if (sglobals->fd_idx[i].type != STCP_FD_UNUSED)
				FD_SET(i, &fds);
		}
		pthread_mutex_unlock(&sglobals->lock);
		...
		do {
			if (FD_ISSET(i, &fds)) {
				stcp_fd_idx_t *idx =
				    (stcp_fd_idx_t *) & sglobals->fd_idx[i];

				if (idx->type == STCP_FD_EP)
					stcp_recvfrom_ep(idx->ep);
				start = i;
			}
			i = (i + 1) % nfds;
		} while (i != start);
