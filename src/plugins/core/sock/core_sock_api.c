...
static inline void
sock_close_socket(cci_os_handle_t sock)
{
    int found = 0;
                           ...
                           cci_connection_t **connection)
{
    ...
    *((struct sockaddr_in *) &sconn->sin) = rx->sin;
    ...
    sock_pack_conn_reply(&hdr_r->header, CCI_EVENT_CONNECT_ACCEPTED,
                         sconn->peer_id);
    sock_pack_seq_ts(&hdr_r->seq_ts, sconn->seq, (uint32_t) sconn->last_ack_ts);
                       ...
                       const struct sockaddr_in sin)
{
    ...
    if (0) sock_sendmsg(sock, NULL, NULL, 0, sin);
...
sock_progress_pending(cci__dev_t *dev)
{
    ...
    TAILQ_FOREACH_SAFE(tx, &sdev->pending, dentry, tmp) {
        ...
        if (ret != tx->len) {
            debug((CCI_DB_MSG|CCI_DB_INFO), "sendto() failed with %s", cci_strerror((enum cci_status)errno));
            continue;
        }
    }
    pthread_mutex_unlock(&dev->lock);
...
sock_progress_queued(cci__dev_t *dev)
{
    ...
    TAILQ_FOREACH_SAFE(tx, &sdev->queued, dentry, tmp) {
        ...
        if (ret == -1) {
            switch (errno) {
            default:
                debug((CCI_DB_MSG|CCI_DB_INFO), "sendto() failed with %s", strerror(errno));
                /* fall through */
            case EINTR:
            case EAGAIN:
            case ENOMEM:
    ...
    TAILQ_FOREACH_SAFE(ack, &sconn->acks, entry, tmp) {
        if (SOCK_SEQ_GTE(seq, ack->start) &&
            ...
            /* seq exists in this entry,
               do nothing */
            debug(CCI_DB_MSG, "%s seq %u exists between %u-%u", __func__,
                  seq, ack->start, ack->end);
            done = 1;
            break;
        } else if (seq == ack->start - 1) {
            /* add it to start of this entry */
            ack->start = seq;
            debug(CCI_DB_MSG, "%s seq %u exists before %u-%u", __func__,
                  seq, ack->start, ack->end);
            done = 1;
            break;
            ...
            debug(CCI_DB_MSG, "%s seq %u exists after %u-%u", __func__,
                  seq, ack->start, ack->end);
            ack->end = seq;
            ...
            if (next) {
                ...
                debug(CCI_DB_MSG, "%s merging acks %u-%u with %u-%u", __func__,
                      ack->start, ack->end, next->start, next->end);
            ...
            if (new) {
                debug(CCI_DB_MSG, "%s seq %u insert after %u-%u before %u-%u ",
                      __func__, seq, last->start, last->end, ack->start, ack->end);
                ...
                uint32_t id)
{
    ...
    if (count == 1) {
        assert(type == SOCK_MSG_ACK_ONLY ||
               type == SOCK_MSG_ACK_UP_TO);
    } else {
    ...
    TAILQ_FOREACH_SAFE(tx, &sconn->tx_seqs, tx_seq, tmp) {
        if (type == SOCK_MSG_ACK_ONLY) {
            ...
            if (SOCK_SEQ_LTE(tx->seq, acks[0])) {
                if (tx->state == SOCK_TX_PENDING) {
                    debug(CCI_DB_MSG, "%s acking tx seq %u (up to seq %u)",
            ...
            for (i = 0; i < count ; i += 2) {
                if (SOCK_SEQ_GTE(tx->seq, acks[i]) &&
                    ...
                    if (sconn->seq_pending == acks[i] - 1)
                        sconn->seq_pending = acks[i + 1];
                    if (tx->state == SOCK_TX_PENDING) {
                        debug(CCI_DB_MSG, "%s sacking seq %u", __func__, acks[0]);
                        found++;
                        TAILQ_REMOVE(&sdev->pending, tx, dentry);
                        TAILQ_REMOVE(&sconn->tx_seqs, tx, tx_seq);
                        if (tx->msg_type == SOCK_MSG_SEND) {
        ...
        if (rma_op && rma_op->status == CCI_SUCCESS) {
            ...
            if (rma_op->next < rma_op->num_msgs) {
                ...
                if (rma_op->msg_len) {
                    ...
                    memcpy(&hdr_r->data, rma_op->msg_ptr, tx->len);
                    tx->len += sizeof(*hdr_r);
                    TAILQ_INSERT_TAIL(&queued, tx, dentry);
                    continue;
                } else {
    ...
    if (!sconn) {
        ...
        if (!sconn) {
            ...
            debug((CCI_DB_CONN|CCI_DB_MSG), "ep %d recv'd conn_reply (%s) from %s"
                  ...
                  reply == CCI_EVENT_CONNECT_ACCEPTED ? "success" : "rejected",
                  from);
...
sock_recvfrom_ep(cci__ep_t *ep)
{
    ...
    if (!rx) {
        ...
        debug(CCI_DB_INFO,
              "no rx buffers available on endpoint %d",
              sep->sock);
        ...
        if (sconn == NULL) {
            /* If the connection is not already established, we just drop the
               message */
            debug (CCI_DB_INFO, "Connection not established, dropping msg\n");
            CCI_EXIT;
            return 0;
        }
    ...
    case SOCK_MSG_RMA_READ_REPLY:
        break;
    default:
        debug(CCI_DB_MSG, "unknown active message with type %u", (enum sock_msg_type)type);
    }
