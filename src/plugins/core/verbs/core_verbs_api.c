...
#if 0
static int
verbs_ifa_by_interface(struct ifaddrs *ifaddrs, const char *interface,
		    ...
		    struct ifaddrs **ifaddr, int *up)
{
	...
	for (ifa = ifaddrs; ifa != NULL; ifa = ifa->ifa_next) {
		...
		if (in.s_addr == s->sin_addr.s_addr) {
			...
			if (ifa->ifa_flags & IFF_LOOPBACK) {
				debug(CCI_DB_INFO, "%s is loopback",
				      ifa->ifa_name);
				goto out;
			}
			*up = ifa->ifa_flags & IFF_UP;
		      ...
		      cci_endpoint_t ** endpoint, cci_os_handle_t * fd)
{
	...
	*((char **)&ep->endpoint.name) = strdup(name);
	...
	vep->tx_mr =
	    ibv_reg_mr(vep->pd, vep->tx_buf, len, IBV_ACCESS_LOCAL_WRITE);
	if (!vep->tx_mr) {
		ret = errno;
		goto out;
	}
	...
	vep->rx_mr =
	    ibv_reg_mr(vep->pd, vep->rx_buf, len, IBV_ACCESS_LOCAL_WRITE);
	if (!vep->rx_mr) {
		ret = errno;
		goto out;
	}
