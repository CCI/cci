...
static inline uint64_t portals_tv_to_usecs(struct timeval tv)
{
 ...
 *
 * transport = portals      # must be lowercase
 *
...
static inline void portals_pack_bits(ptl_match_bits_t * match_bits,
				     portals_msg_type_t type, uint64_t a)
{

	*match_bits = PORTALS_PACK_BITS(type, a);
}

static inline void portals_parse_match_bits(ptl_match_bits_t match_bits,
