...
static inline void portals_sample_init(void)
{
	...
	portals_start_ns =
	    calloc(PORTALS_NUM_SAMPLES, sizeof(*portals_start_ns));
 ...
 * Local functions
 */
static int ctp_portals_init(cci_plugin_ctp_t *plugin, uint32_t abi_ver, uint32_t flags, uint32_t * caps);
static int ctp_portals_finalize(cci_plugin_ctp_t *plugin);
static const char *ctp_portals_strerror(cci_endpoint_t * endpoint, enum cci_status status);
static int ctp_portals_create_endpoint(cci_device_t * device,
				   int flags,
...
static int ctp_portals_accept(cci_event_t *event,
			  const void *context);
static int ctp_portals_reject(cci_event_t *event);
static int ctp_portals_connect(cci_endpoint_t * endpoint,
...
static int ctp_portals_get_event(cci_endpoint_t * endpoint,
			     cci_event_t ** event);
static int ctp_portals_return_event(cci_event_t * event);
static int ctp_portals_send(cci_connection_t * connection,
			const void *ptr, uint32_t len, const void *context, int flags);
static int ctp_portals_sendv(cci_connection_t * connection,
			 const struct iovec *data,
			 uint32_t iovcnt, const void *context, int flags);
static int ctp_portals_rma_register(cci_endpoint_t * endpoint,
				void *start, uint64_t length,
				int flags, uint64_t * rma_handle);
static int ctp_portals_rma_deregister(cci_endpoint_t * endpoint, uint64_t rma_handle);
static int ctp_portals_rma(cci_connection_t * connection,
		       const void *msg_ptr,
		       uint32_t msg_len,
 ...
 * plugin resides.
 */
cci_plugin_ctp_t cci_ctp_portals_plugin = {
	{
	 /* Logistics */
	 ...
	 50,
	 cci_ctp_portals_post_load,	/* Bootstrap function pointers */
	 cci_ctp_portals_pre_unload,
	 },
	...
	ctp_portals_reject,
	ctp_portals_connect,
	ctp_portals_disconnect,
	ctp_portals_set_opt,
	ctp_portals_get_opt,
	ctp_portals_arm_os_handle,
	ctp_portals_get_event,
	ctp_portals_return_event,
	ctp_portals_send,
	ctp_portals_sendv,
	ctp_portals_rma_register,
	ctp_portals_rma_deregister,
	ctp_portals_rma
};
...
/*      Save off portals ID of device. */
		...
		pdev->ep_idxs =
		    calloc(PORTALS_NUM_BLOCKS, sizeof(*pdev->ep_idxs));
		if (!pdev->ep_idxs) {
			free(pglobals->devices);
			free(pglobals);
			pglobals = NULL;
		...
		for (arg = device->conf_argv; *arg != NULL; arg++) {
			...
			if (!strncmp("min_idx=", *arg, 8)) {
				...
				if (pdev->min_idx >= pdev->max_pt_index) {
					debug(CCI_DB_WARN,
					      "requested min_idx (%u) is larger than the "
					      "max_pt_index (%d)",
					      pdev->min_idx,
					      pdev->max_pt_index);
...
/*  Try to create progress thread. */
	iRC =
	    pthread_create(&progress_tid, NULL, portals_progress_thread, NULL);
	if (iRC) {		/* Failed */
...
/* NOTE never return 0 */
static void portals_get_ep_idx(portals_dev_t * pdev, uint32_t * idx)
{
	...
	for (i = pdev->min_idx; i < pdev->max_pt_index; i++) {
		...
		if ((*b & (1ULL << offset)) == 0) {
			...
			*idx = (block * PORTALS_BLOCK_SIZE) + offset;
			debug(CCI_DB_CONN,
			      "getting EP idx %u block=%" PRIx64 "", *idx, *b);
			break;
		}
	}
	return;
...
static void portals_put_ep_idx(portals_dev_t * pdev, uint32_t idx)
{
 ...
 */
static int portals_free_tx(portals_ep_t * pep, int force)
{
	portals_tx_t *tx;
...
/* Caller must be holding ep->lock */
static int portals_add_tx(cci__ep_t * ep)
{
	int ret = 1;
	ptl_md_t md;
...
/* Caller must be holding ep->lock */
static int portals_add_rx(cci__ep_t * ep)
{
	int ret = 1;
	portals_ep_t *pep = ep->priv;
	portals_rx_t *rx;
 ...
 * Caller must be holding ep->lock.
 */
static int portals_free_rx(portals_ep_t * pep, int force)
{
	portals_rx_t *rx;
...
static int portals_post_am_buffer(cci__ep_t * ep, portals_am_buffer_t * am)
{
	int ret = CCI_SUCCESS;
	...
	ret = PtlMEMDAttach(pdev->niHandle,
			    ...
			    PTL_UNLINK,
			    PTL_INS_AFTER,
			    am->md, PTL_UNLINK, &(am->meh), &(am->mdh));
	if (ret != PTL_OK) {
		...
		default:	/* Undocumented portals error */
			debug(CCI_DB_WARN, "PtlMEMDAttach() returned %s",
			      ptl_err_str[ret]);
			ret = CCI_ERROR;
		}
	} else {
...
static int portals_create_am_buffer(cci__ep_t * ep, uint64_t length)
{
	int ret = CCI_SUCCESS;
	portals_ep_t *pep = ep->priv;
	...
	if (pep) {
		...
		while (!TAILQ_EMPTY(&pep->ams)) {
			...
			if (am->buffer) {
				...
				debug(CCI_DB_MEM, "Free AM buffer=%p",
				      am->buffer);
				free(am->buffer);
				am->buffer = NULL;
			}
			  ...
			  const void *context)
{
	...
	request =
	    (portals_conn_request_t *) (rx->pevent.md.start +
					rx->pevent.offset);
	...
	debug(CCI_DB_CONN, "accept.max_recv_buffer_count = %u",
	      accept.max_recv_buffer_count);
	debug(CCI_DB_CONN, "accept.server_conn_upper = 0x%x",
	      accept.server_conn_upper);
	debug(CCI_DB_CONN, "accept.server_conn_lower = 0x%x",
	      accept.server_conn_lower);
	...
	ret = PtlPutRegion(tx->mdh,	/* Handle to MD */
			   0, ac_len, PTL_NOACK_REQ,	/* ACK disposition */
...
static int ctp_portals_reject(cci_event_t *event)
{
	...
	ret = PtlPut(mdh,
		     PTL_NOACK_REQ,
		     rx->pevent.initiator,
		     idx, 0, bits, 0, (uintptr_t) rx->pevent.hdr_data);
	if (ret != PTL_OK) {
		switch (ret) {
		case PTL_NO_INIT:
			ret = CCI_ENODEV;
...
static int portals_getaddrinfo(const char *uri,
			       ptl_process_id_t * idp, uint32_t * idx)
{
	...
	if (!strncmp(PORTALS_URI, uri, len)) {	/* URI to portals ID */
		...
		if (cp)
			*cp = '\0';	/* local overwrite only */
			...
			debug(CCI_DB_INFO,
			      "%s: cannot parse %s - no NID:PID colon",
			      __func__, uri);
			free(addr);
			return CCI_EINVAL;
		}
		...
		if (cp)
			*cp = '\0';	/* local overwrite only */
			...
			debug(CCI_DB_INFO,
			      "%s: cannot parse %s - no PID:IDX colon",
			      __func__, uri);
			free(addr);
			return CCI_EINVAL;
		}
		...
		*idx = atoi(cp);
		...
		debug(CCI_DB_INFO, "%s: cannot parse %s (base %s len %d)",
		      __func__, uri, PORTALS_URI, len);
		CCI_EXIT;
		return CCI_EINVAL;
	}
 ...
 */
static int portals_set_ep_tx_buf_cnt(cci__ep_t * ep, uint32_t new_count)
{
	int i;
...
/* Caller must be holding ep->lock */
static int portals_orphan_am_buffer(cci__ep_t * ep, portals_am_buffer_t * am)
{
	int ret = PTL_OK;
	portals_ep_t *pep = ep->priv;
 ...
 * to the number left which might be higher than new_count.
 */
static int portals_set_ep_rx_buf_cnt(cci__ep_t * ep, uint32_t new_count)
{
	int ret = CCI_SUCCESS;
	int i;
	...
	TAILQ_FOREACH(e, &ep->evts, entry) {
		if (e->event.type == CCI_EVENT_SEND) {
			if (e->priv) {
				 ...
				 */
				portals_tx_t *tx =
				    container_of(e, portals_tx_t, evt);
			       ...
			       int flags, portals_rma_op_t * rma_op)
{
	...
	if (data_len) {
		...
		for (i = 0; i < iovcnt; i++) {
			memcpy(tx->buffer + offset, data[i].iov_base,
			       data[i].iov_len);
			 ...
			 uint32_t iovcnt, const void *context, int flags)
{
	...
	ret =
	    portals_send_common(connection, data, iovcnt, context, flags, NULL);
			...
			uint32_t msg_len, const void *context, int flags)
{
	...
	ret =
	    portals_send_common(connection, &iov, iovcnt, context, flags, NULL);
	CCI_EXIT;
	return ret;
}
...
//	handle->conn = conn;
	...
	iRC = PtlMEMDAttach(pdev->niHandle,
			    ...
			    PTL_RETAIN,
			    PTL_INS_AFTER,
			    md, PTL_RETAIN, &handle->meh, &handle->mdh);
	if (iRC != PTL_OK) {
		       ...
		       uint64_t data_len, const void *context, int flags)
{
	...
	portals_conn_t *pconn = NULL;
	portals_rma_handle_t *local =
	    (portals_rma_handle_t *) ((uintptr_t) local_handle);
	portals_rma_op_t *rma_op = NULL;
	...
	if (flags & CCI_FLAG_WRITE) {
		ret = PtlPutRegion(local->mdh,	/* Handle to MD */
				   ...
				   0);	/* hdr_data */
		debug(CCI_DB_MSG,
		      "%s: RMA WRITE bits=0x%" PRIx64 " offset=%" PRIu64
		      " returned=%s len=%" PRIu64 "", __func__,
		      remote_handle | PORTALS_MSG_RMA_WRITE, remote_offset,
		      ptl_err_str[ret], data_len);
		if (ret == PTL_OK) {
			ret = CCI_SUCCESS;
		} else {
			ret = CCI_ERROR;
		...
		ret = PtlGetRegion(local->mdh,	/* Handle to MD */
				   ...
				   remote_offset);	/* remote offset */
		debug(CCI_DB_MSG,
		      "%s: RMA READ bits=0x%" PRIx64 " offset=%" PRIu64
		      " returned=%s len=%" PRIu64 "", __func__,
		      remote_handle | PORTALS_MSG_RMA_READ, remote_offset,
		      ptl_err_str[ret], data_len);
		if (ret == PTL_OK) {
			ret = CCI_SUCCESS;
		} else {
...
static void portals_handle_conn_request(cci__ep_t * ep, ptl_event_t pevent)
{
	...
	if (am->length - (pevent.offset + pevent.mlength) <
	    dev->device.max_send_size) {
		int active = 0;
		portals_am_buffer_t *a;

		am->state = PORTALS_AM_INACTIVE;
		debug((CCI_DB_INFO | CCI_DB_MSG),
		      "%s: unlinking active message buffer", __func__);
		...
		if (!active)
			debug(CCI_DB_WARN,
			      "both active message buffers inactive");
	}
	...
	if (!rx) {
		debug((CCI_DB_WARN | CCI_DB_MSG),
		      "no rx available for incoming CONN_REQ");
		return;
	}
	...
	*((uint32_t *) & rx->evt.event.request.data_len) =
	    (pevent.match_bits >> 16) & 0xFFFF;
	if (rx->evt.event.request.data_len)
		*((void **)&rx->evt.event.request.data_ptr) =
		    pevent.md.start + pevent.offset +
		    (uintptr_t) sizeof(portals_conn_request_t);
	else
		*((void **)&rx->evt.event.request.data_ptr) = NULL;

	debug(CCI_DB_CONN,
	      "%s: from " PORTALS_URI "%u:%hu:%u client_conn=0x%llx", __func__,
	      pevent.initiator.nid, pevent.initiator.pid,
	      ((portals_conn_request_t *) (pevent.md.start +
					   pevent.offset))->client_ep_idx,
...
static void portals_handle_conn_reply(cci__ep_t * ep, ptl_event_t pevent)
{
	...
	portals_rx_t *rx = NULL;
	portals_conn_accept_t *accept =
	    (void *)(pevent.md.start) + pevent.offset;
	cci__evt_t *evt = NULL;
	cci__dev_t *dev = ep->dev;
	portals_am_buffer_t *am = pevent.md.user_ptr;
	...
	if (!rx) {
		debug((CCI_DB_WARN | CCI_DB_MSG),
		      "no rx available for incoming conn_reply");
		return;
	}
...
static void portals_handle_active_msg(cci__ep_t * ep, ptl_event_t pevent)
{
	...
	if (!rx) {
		debug((CCI_DB_WARN | CCI_DB_MSG),
		      "no rx available for incoming AM");
		return;
	}
	...
	*((uint32_t *) & evt->event.recv.len) =
	...
	debug(CCI_DB_MSG, "%s: recv'd len=%d ptr=%p",
	      __func__, evt->event.recv.len, evt->event.recv.ptr);
...
static void
portals_complete_rma(portals_rma_op_t * rma_op, portals_rma_handle_t * local,
		     ptl_event_t event)
{
...
static void portals_get_event_ep(cci__ep_t * ep)
{
	...
	debug(CCI_DB_INFO, "%s: got portals %s event", __func__,
	      ptl_event_str[event.type]);
	switch (event.type) {
	case PTL_EVENT_SEND_START:
		debug((CCI_DB_WARN | CCI_DB_INFO),
		      "we missed disabling a portals send_start");
		break;
	case PTL_EVENT_SEND_END:
		{
			portals_msg_type_t msg_type =
			    (enum portals_msg_type)(event.match_bits & 0x3);
			int is_reliable = (int)(event.match_bits & 0x4);
			portals_tx_t *tx = (void *)event.md.user_ptr;

			if (msg_type == PORTALS_MSG_RMA_WRITE ||
			    msg_type == PORTALS_MSG_RMA_READ || is_reliable) {
				/* wait for the ACK instead */
				debug(CCI_DB_MSG,
				      "%s: ignoring SEND_END for reliable %s",
				      __func__,
				      msg_type ==
				      PORTALS_MSG_SEND ? "SEND" : msg_type ==
				      PORTALS_MSG_RMA_WRITE ? "RMA WRITE" :
				      msg_type ==
				      PORTALS_MSG_RMA_READ ? "RMA READ" :
				      "OOB");
				break;
			}
			...
			case PORTALS_MSG_SEND:
				debug(CCI_DB_MSG, "%s: send_end for SEND",
				      __func__);
				if (tx->flags & CCI_FLAG_SILENT
				    || tx->flags & CCI_FLAG_BLOCKING) {
					/* queue on idle_txs */
					pthread_mutex_lock(&ep->lock);
					TAILQ_INSERT_HEAD(&pep->idle_txs, tx,
							  dentry);
					pthread_mutex_unlock(&ep->lock);
				} else {
					...
					tx->evt.event.send.status =
					    event.ni_fail_type ==
					    PTL_NI_OK ? CCI_SUCCESS : CCI_ERROR;
					pthread_mutex_lock(&ep->lock);
					TAILQ_INSERT_TAIL(&ep->evts, &tx->evt,
							  entry);
					pthread_mutex_unlock(&ep->lock);
				}
				break;
			default:
				debug(CCI_DB_INFO,
				      "we missed disabling a portals send_end for "
				      "msg_type %u",
				      (enum portals_msg_type)(tx->msg_type));
				break;
			}
	...
	case PTL_EVENT_PUT_END:
		{
			...
			case PORTALS_MSG_OOB:
				{
					/* incoming OOB msg */
					portals_msg_oob_type_t oob_type =
					    (enum
					     portals_msg_oob_type)((event.
								    match_bits
								    >> 2) &
								   0x3);
					...
					case PORTALS_MSG_OOB_CONN_REQUEST:
						portals_handle_conn_request(ep,
									    event);
						break;
					case PORTALS_MSG_OOB_CONN_REPLY:
						portals_handle_conn_reply(ep,
									  event);
						break;
					default:
						debug(CCI_DB_INFO,
	...
	case PTL_EVENT_REPLY_END:
		{
			portals_msg_type_t msg_type =
			    (portals_msg_type_t) (event.match_bits & 0x3);
			//portals_tx_t *tx = NULL;
			portals_rma_handle_t *handle = NULL, *local = NULL;
			portals_rma_op_t *rma_op = NULL, *ro = NULL, *tmp =
			    NULL;
			...
			TAILQ_FOREACH_SAFE(ro, &handle->rma_ops, hentry, tmp) {
				...
				if (event.match_bits ==
					...
					TAILQ_REMOVE(&handle->rma_ops, ro,
						     hentry);
					rma_op = ro;
					break;
				} else
					debug(CCI_DB_WARN,
					      "match=%" PRIx64 "..%" PRIx64
					      "  length=%" PRIu64 "..%" PRIu64
					      "  offset=%" PRIu64 "..%" PRIu64
					      "", (uint64_t) event.match_bits,
					      (uint64_t) (ro->
							  remote_handle |
							  PORTALS_MSG_RMA_READ),
					      (uint64_t) event.rlength,
					      (uint64_t) ro->data_len,
					      (uint64_t) event.offset,
					      (uint64_t) ro->remote_offset);
			...
			if (rma_op->msg_len) {
				...
				ret =
				    portals_send_common(&local->conn->
							connection, &msg, 1,
							rma_op->context,
							rma_op->flags, rma_op);
	...
	case PTL_EVENT_ACK:
		{
			...
			if (event.ni_fail_type != PTL_NI_OK)
				debug(CCI_DB_WARN,
				      "%s: send failed with ni_fail_type %u",
			...
			case PORTALS_MSG_SEND:
				...
				if (tx->flags & CCI_FLAG_SILENT || rma_op) {
					...
					tx->evt.event.send.status =
					    ...
					    PTL_NI_OK ? CCI_SUCCESS : CCI_ERROR;
					pthread_mutex_lock(&ep->lock);
					TAILQ_INSERT_HEAD(&ep->evts, &tx->evt,
							  entry);
					pthread_mutex_unlock(&ep->lock);
				}
				if (rma_op) {
					portals_rma_handle_t *local =
					    (void *)rma_op->local_handle;
					portals_complete_rma(rma_op, local,
							     event);
				}
				break;
			case PORTALS_MSG_RMA_WRITE:
				{
					...
					handle = event.md.user_ptr;
					TAILQ_FOREACH_SAFE(ro, &handle->rma_ops,
							   hentry, tmp) {
						portals_conn_t *pconn =
						    ro->evt.conn->priv;
						if (event.match_bits ==
					...
					if (!rma_op) {
						debug(CCI_DB_WARN,
						      "%s: unable to find rma_op for RMA WRITE "
						      "completion", __func__);
						break;
					}
			...
			default:
				debug(CCI_DB_INFO,
				      "we missed disabling a portals ack for "
				      "msg_type %u",
				      (enum portals_msg_type)tx->msg_type);
				break;
			}
	...
	case PTL_EVENT_UNLINK:
		...
		break;
	default:
		debug(CCI_DB_INFO, "unexpected portals event %u",
		      (enum cci_event_type)event.type);
		break;
	}
