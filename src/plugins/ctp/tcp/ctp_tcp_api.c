...
static inline int tcp_create_threads (cci__ep_t *ep)
{
	int ret;
	tcp_ep_t *sep;
	...
	ret = pthread_create(&sep->recv_tid, NULL, tcp_recv_thread, (void*)ep);
	if (ret)
		goto out;

	ret = pthread_create(&sep->progress_tid, NULL, tcp_progress_thread, (void*)ep);
	if (ret)
		goto out;
...
static inline int tcp_terminate_threads (tcp_ep_t *sep)
{
	assert (sep);
...
static inline int
tcp_set_nonblocking(cci_os_handle_t sock, tcp_fd_type_t type, void *p)
{
	int ret, flags;
...
static int tcp_sendmsg(cci_os_handle_t sock, struct iovec iov[2],
		       ...
		       const struct sockaddr_in sin)
{
	...
	ret = tcp_sendmsg(sock, iov, count, sin);
	if (ret != -1)
		assert(ret == len);
...
static void tcp_progress_pending(cci__ep_t * ep)
{
	...
	TAILQ_FOREACH_SAFE(evt, &sep->pending, entry, tmp) {
			...
			case TCP_MSG_CONN_REQUEST:
				{
					...
					i = tcp_ip_hash(sconn->sin.sin_addr.
							 s_addr, 0);
					active_list = &sep->active_hash[i];
					pthread_mutex_lock(&ep->lock);
		...
		ret = tcp_sendto(sep->sock, tx->buffer, tx->len, tx->rma_ptr, tx->rma_len, sconn->sin);
		if (ret != tx->len) {
...
static inline int 
pack_piggyback_ack (cci__ep_t *ep, tcp_conn_t *sconn, tcp_tx_t *tx)
{
    tcp_ack_t *ack = NULL;
    uint64_t now = 0ULL;
				   ...
				   struct sockaddr_in sin, cci__ep_t * ep)
{
	...
	if (!sconn) {
		...
		if (!sconn) {
			...
			ret = tcp_sendto(sep->sock, &hdr, len, NULL, 0, sin);
			if (ret != len) {
				debug((CCI_DB_CONN | CCI_DB_MSG),
...
static inline void
lookup_contextid(tcp_conn_t * sconn, uint64_t context_id, void **context)
{
...
static int tcp_recvfrom_ep(cci__ep_t * ep)
{
	...
	if (sconn && cci_conn_is_reliable(sconn->conn) &&
		...
		tcp_handle_seq(sconn, seq);
        if (hdr_r->pb_ack != 0)
            tcp_handle_ack (sconn, type, rx, 1, id);
	}
...
out:
	...
	if (drop_msg) {
		if (cci_conn_is_reliable(sconn->conn) && sconn->rnr == seq) {
			...
			tcp_sendto(sep->sock, buffer, len, NULL, 0, sconn->sin);
		}
...
static void tcp_keepalive(cci__ep_t *ep)
{
	...
	sdev = dev->priv;
	sep = ep->priv;
	i = tcp_ip_hash(sdev->ip, sdev->port);
	conn_list = &sep->conn_hash[i];
			...
			TAILQ_FOREACH(sconn, &sep->conn_hash[i], entry) {
				if (!TAILQ_EMPTY(&sconn->acks)) {
					...
					if (1 == tcp_need_sack(sconn)) {
						...
						ack = TAILQ_FIRST(&sconn->acks);
                        if (TCP_U64_LT(now, sconn->last_ack_ts + ACK_TIMEOUT)
