    ...
    *plugin_best = NULL;
    for (i = 0; NULL != cci_plugins_filename_cache && 
        ...
        if (strncasecmp(ptr, prefix, prefix_len) == 0) {
            if (open_plugin(cci_plugins_filename_cache[i], &handle, &plugin, 
                            verify) == CCI_SUCCESS) {
                /* First possibillity of a plugin */
                if (NULL == *plugin_best) {
                    if (NULL != plugin->post_load &&
                        CCI_SUCCESS != plugin->post_load(plugin)) {
                        fprintf(stderr, "Post load hook for %s failed -- ignored\n", ptr);
                        lt_dlclose(handle);
                        continue;
                    }
                ...
                /* This new plugin is better than the old one */
                else if (plugin->priority > (*plugin_best)->priority) {
                    if (NULL != plugin->post_load &&
                        CCI_SUCCESS != plugin->post_load(plugin)) {
                        fprintf(stderr, "Post load hook for %s failed -- ignored\n", ptr);
                        lt_dlclose(handle);
                    ...
                    if (NULL != (*plugin_best)->pre_unload) {
                        (*plugin_best)->pre_unload(*plugin_best);
                    }
                       ...
                       cci_plugins_framework_verify_fn_t verify)
{
    ...
    *plugin = lt_dlsym(*handle, struct_name);
    if (NULL == *plugin) {
        fprintf(stderr, "Unable to find \"%s\" symbol in %s -- ignored\n", 
    ...
    if ((*plugin)->cci_abi_version != CCI_ABI_VERSION) {
        fprintf(stderr, "Plugin \"%s\" in %s supports ABI version %d; only version %d is supported -- ignored\n",
                (*plugin)->plugin_name, filename, (*plugin)->cci_abi_version,
                CCI_ABI_VERSION);
        goto bad;
    }
